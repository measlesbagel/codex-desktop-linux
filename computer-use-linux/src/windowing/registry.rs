...
pub const WINDOW_PERMISSION_HINT: &str = "Computer Use could not access a supported window list backend. Targeted window input requires session-bus access plus GNOME Shell Introspect, the Codex GNOME Shell extension, the COSMIC Wayland helper, KWin/Plasma DBus scripting, niri msg, Hyprland hyprctl, or i3-msg. On GNOME, run setup_window_targeting to install the extension backend.";
...
const DESCRIPTORS: &[BackendDescriptor] = &[
    ...
    BackendDescriptor {
        ...
        missing_hint: "On niri, ensure niri msg can reach the active compositor IPC socket.",
        can_exact_focus: true,
    },
    BackendDescriptor {
        id: HYPRLAND_BACKEND,
